...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'accept') {
        ...
        if ($request) {
            ...
            // Determine party name (could be 'party' or 'party_name' depending on table)
            ...
            // Insert as candidate with all fields
            $insert = $conn->prepare("INSERT INTO candidates (id, name, position, party_name, party_image, faculty, class, photo, platform) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
            $insert->bind_param("issssssss", $next_id, $request['candidate_name'], $request['position'], $party_name, $request['party_image'], $user['faculty'], $user['class'], $request['photo'], $request['vision']);
            $insert->execute();
