...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (in_array('party', $columns) && in_array('party_image', $columns) && in_array('photo', $columns)) {
        // Full table with party name and both image columns
        $stmt = $conn->prepare("INSERT INTO candidate_requests (voter_id, candidate_name, position, party, party_image, photo, vision, request_time) VALUES (?, ?, ?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("issssss", $voter_id, $candidate_name, $position, $party, $party_image_path, $photo_path, $vision);
    } elseif (in_array('party_image', $columns) && in_array('photo', $columns)) {
        // Table with both image columns but no party field
