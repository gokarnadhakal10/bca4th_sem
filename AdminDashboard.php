...
if (isset($vote_session['status']) && in_array($vote_session['status'], ['Active', 'Paused', 'Scheduled'])) {
...
if (isset($vote_session['status']) && $vote_session['status'] === 'Scheduled' && strtotime($vote_session['start_time']) <= strtotime($current_time_for_auto_update)) {
...
if (isset($nomination_session['status']) && $nomination_session['status'] === 'Scheduled' && strtotime($nomination_session['start_time']) <= strtotime($current_time_for_auto_update)) {
...
$is_nomination_running = in_array($nomination_status, ['Active', 'Paused', 'Scheduled']);
$is_voting_running = in_array($voting_status, ['Active', 'Paused', 'Scheduled']);
...
if (!empty($notice['published_at'])) {
    echo date('M d, Y', strtotime($notice['published_at']));
} else {
    echo 'No Date';
}
?>
            ...
            <form action="manage_session.php" method="post" class="control-form" style="grid-template-columns: 1fr;">
                ...
                <div class="form-group" style="display: flex; gap: 5px; flex-wrap: wrap;">
                    <?php if (!$is_nomination_running): ?>
                        ...
                        <button type="submit" name="action" value="update" class="btn btn-primary" style="flex: 1;"><i class="fas fa-sync-alt"></i> Update</button>
                        
                        <?php if($vote_session['status'] == 'Paused'): ?>
                            <button type="submit" name="action" value="resume" class="btn btn-info" style="flex: 1;"><i class="fas fa-redo"></i> Resume</button>
                        <?php else: ?>
                            <button type="submit" name="action" value="pause" class="btn btn-warning" style="flex: 1;" <?= (in_array($vote_session['status'], ['Pending', 'Ended', 'Scheduled'])) ? 'disabled' : '' ?>><i class="fas fa-pause"></i> Pause</button>
                        <?php endif; ?>
                    ...
                    <?php else: ?>
                        ...
                        <button type="button" class="btn btn-primary disabled" style="flex: 1;" title="Nomination is active" disabled><i class="fas fa-sync-alt"></i> Update</button>
            ...
            <form action="manage_nomination.php" method="post" class="control-form" style="grid-template-columns: 1fr;">
                ...
                <div class="form-group" style="display: flex; gap: 5px; flex-wrap: wrap;">
                    <?php if (!$is_voting_running): ?>
                        ...
                        <?php if($nomination_session['status'] == 'Paused'): ?>
                            <button type="submit" name="action" value="resume" class="btn btn-info" style="flex: 1;"><i class="fas fa-redo"></i> Resume</button>
                        <?php else: ?>
                            <button type="submit" name="action" value="pause" class="btn btn-warning" style="flex: 1;" <?= (in_array($nomination_session['status'], ['Pending', 'Ended', 'Scheduled'])) ? 'disabled' : '' ?>><i class="fas fa-pause"></i> Pause</button>
                        <?php endif; ?>
                    ...
                    <?php else: ?>
                        ...
                        <button type="button" class="btn btn-primary disabled" style="flex: 1;" title="Voting is active" disabled><i class="fas fa-sync-alt"></i> Update</button>
    ...
    <div class="dashboard-section">
        <div class="section-header">
            ...
            <?php if ($can_add_candidate): ?>
                <a href="add_candidates.php" class="btn btn-primary"><i class="fas fa-plus"></i> Add New</a>
            <?php else: ?>
                <a href="#" class="btn btn-primary disabled" onclick="alert('Adding candidates is only allowed during the active nomination period.'); return false;" title="Adding candidates is only allowed during the active nomination period."><i class="fas fa-plus"></i> Add New</a>
            <?php endif; ?>
        </div>
        <div class="table-responsive">
            <table>
    ...
    <div class="dashboard-section">
        ...
        <div class="quick-actions">
            <div class="action-card">
                ...
                <?php else: ?>
                    <a href="#" class="btn btn-success disabled" onclick="alert('Adding candidates is only allowed during the active nomination period.'); return false;" title="Adding candidates is only allowed during the active nomination period.">Go to Add</a>
                <?php endif; ?>
            </div>
            <div class="action-card">
