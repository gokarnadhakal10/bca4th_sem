...
if ($vote_session && $vote_session['status'] === 'Scheduled' && strtotime($vote_session['start_time']) <= strtotime($current_time)) {
    $conn->query("UPDATE voting_session SET status = 'Active' WHERE id = 1");
    $vote_session['status'] = 'Active';
}
...
if ($nom_session && $nom_session['status'] === 'Scheduled' && strtotime($nom_session['start_time']) <= strtotime($current_time)) {
    $conn->query("UPDATE nomination_session SET status = 'Active' WHERE id = 1");
    $nom_session['status'] = 'Active';
}
