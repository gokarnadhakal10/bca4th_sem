...
if (isset($_POST['add'])) {
    ...
    // Check for duplicate candidate - REMOVED student_id from check
    ...
    if($check_result->num_rows > 0){
        ...
        if($upload_success){
            ...
            $check_platform = $conn->query("SHOW COLUMNS FROM candidates LIKE 'platform'");
            $has_platform = ($check_platform && $check_platform->num_rows > 0);
            
            // Insert candidate based on available columns
            if ($has_platform) {
                ...
                // Without platform column
                $stmt = $conn->prepare("INSERT INTO candidates(id, name, position, party_name, party_image, faculty, class, photo) VALUES(?,?,?,?,?,?,?,?)");
                $stmt->bind_param("isssssss", $next_id, $name, $position, $party_name, $party_image_path, $faculty, $class, $photo_path);
            }
...
<form method="POST" enctype="multipart/form-data">
    ...
    <div class="form-group">
        <label for="platform">Platform/Manifesto (Optional):</label>
