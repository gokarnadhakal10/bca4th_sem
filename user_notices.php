        ...
        <aside class="sidebar">
            <div class="profile-section">
                <div class="profile-avatar">
                    ...
                    <?php else: ?>
                        <?= strtoupper(substr($voter['name'] ?? 'U', 0, 1)); ?>
                    <?php endif; ?>
                </div>
            ...
            <div class="notices-grid">
                <?php if($notices->num_rows > 0): 
                    ...
                    while($row = $notices->fetch_assoc()): 
                        ...
                        // FIX: Safe published and expires date
                        $pub_day = !empty($row['published_at']) ? date('d', strtotime($row['published_at'])) : '--';
                        $pub_month = !empty($row['published_at']) ? date('M', strtotime($row['published_at'])) : '--';
                        $expires = !empty($row['expires_at']) ? date('M d', strtotime($row['expires_at'])) : '';
                ?>
                <div class="notice-card" style="animation-delay: <?= $delay ?>s;">
                    <div class="notice-header">
                        <div class="notice-date">
                            <span class="date-day"><?= $pub_day ?></span>
                            <span class="date-month"><?= $pub_month ?></span>
                        </div>
                        <div class="notice-badges">
                            <span class="badge <?= $cat_class ?>"><?= htmlspecialchars($row['category'] ?? 'General') ?></span>
                        </div>
                    </div>
                    ...
                    <div class="notice-footer">
                        <div class="priority-indicator <?= $prio_class ?>">
                            <i class="fas <?= $prio_icon ?>"></i> <?= !empty($row['priority']) ? ucfirst($row['priority']) : 'Low' ?> Priority
                        </div>
                        <?php if($expires): ?>
                        <div title="Expires on">
                            <i class="far fa-clock"></i> <?= $expires ?>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
