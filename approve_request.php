...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'accept') {
        ...
        if ($request) {
            ...
            // ===== NEW FEATURE: Check if party already has candidate for this position =====
            $check_stmt = $conn->prepare("SELECT COUNT(*) AS count FROM candidates WHERE party_name=? AND position=?");
            $check_stmt->bind_param("ss", $party_name, $request['position']);
            $check_stmt->execute();
            $result = $check_stmt->get_result()->fetch_assoc();

            if ($result['count'] > 0) {
                // Party already has a candidate for this position
                $_SESSION['error'] = "This party already has a candidate for the position '{$request['position']}'. Please register from another party.";
            } else {
                // Find the lowest available ID (Gap Detection)
                $id_result = $conn->query("SELECT id FROM candidates ORDER BY id ASC");
                $next_id = 1;
